...
class BasicLBM:
    ...
    def velocity(self, f, density):
        ...
        uy = (f[2, :, :] + f[5, :, :] + f[6, :, :] - f[4, :, :] - f[7, :, :] - f[8, :, :])/density
        return ux, uy
    ...
    def update_eq(self, feq, density, ux, uy):
        ...
        feq[6, :, :] = w5 * density * (1 - 3*(ux-uy) - 9*ux*uy + 3*u)
        feq[7, :, :] = w5 * density * (1 - 3*(ux+uy) + 9*ux*uy + 3*u)
        feq[8, :, :] = w5 * density * (1 + 3*(ux-uy) - 9*ux*uy + 3*u)
    ...
    def collide(self, f, feq, omega, density, ux, uy, boundary=False,):
        ...
        if boundary is False:
            ...
            if boundary.constant_array is not False:
                ...
                f[5, i] = -self.tmp[7, i] + 1/18*boundary.constant_values[i]
                f[6, i] = -self.tmp[8, i] + 1/18*boundary.constant_values[i]
                f[7, i] = -self.tmp[5, i] + 1/18*boundary.constant_values[i]
                f[8, i] = -self.tmp[6, i] + 1/18*boundary.constant_values[i]
                f[0, i] = -self.tmp[0, i] + 8/9*boundary.constant_values[i]

...
class DMDBase(object):
    ...
    @property
    def cont_spectral(self):
        """
        fourier spectrum of modes
        Get the continous spectral of A tilde.

        :return: the continous spectral from the eigendecomposition of `atilde`. mu = lambda / dt
        :rtype: numpy.ndarray
        """
        return np.log(self.eigs) / self.original_time['dt']
    ...
    def dynamics_norm(self, norm=False):
        """
        Get the time evolution of each mode.
        ...
        if not norm:
            ...
            im = dy.imag
            # im[im<tol] = 0
            zero_ele = np.zeros_like(re[0])
            re_new = np.zeros_like(re)
            im_new = np.zeros_like(im)
            for i in range(re.shape[0]):
                
                max_re = np.absolute(re[i, :]).max()
                max_im = np.absolute(im[i, :]).max()
                # print(max_re, max_im)
                if max_re <= tol:
                    re_new[i, :] = zero_ele
                else:
                    re_new[i, :] = re[i, :]/max_re
                if max_im <= tol:
                    im_new[i, :] = zero_ele
                else:
                    im_new[i, :] = im[i, :]/max_im
    ...
    def reconstructed_data(self):
        """
        Get the reconstructed data only real part.
    ...
    def reconstructed_data_full(self):
        """
        Get the reconstructed data both imag and real part.
        :return: the matrix that contains the reconstructed snapshots.
        :rtype: numpy.ndarray 
        """
        print(f'shape of dynamics {self.dynamics.shape}')
    ...
    def partial_reconstructed_data(self, idx, out_mode='save_reshape'):
        ...
        if out_mode == 'save_reshape': 
            return np.reshape(np.outer(self.modes[:, idx], self.dynamics[idx, :]).real, (self.ny, self.nx, -1))
        if out_mode == 'no_reshape': 
    ...
    def freq_su(self, D=0.0048, U=318.31588):
        """Strouhal number"""
        # for Re =100
        # dt = 0.2
        # T = 300 * dt
    ...
    @property
    def _energy(self):
        E = np.linalg.norm(self.reconstructed_data, axis=0) ** 2
        return E
    @property
    def _power(self):
        # power = np.array([np.linalg.norm(self.modes[:, i], 2) ** 2 for i in
        #                 range(self.modes.shape[1])])
        power = np.array([np.linalg.norm(self.modes[:, i], 2) ** 2 for i in
                        range(self.modes.shape[1])]) * self.frequency
        return power
    ...
    def _sort(self, sort_mode='amp'):
        ...
        if sort_mode not in ['frequencies', 'power', 'amp', 'cri16', 'eigmag', 'energy',None]: raise ValueError
        # assert(len(self.frequency) == len(self._power))

        if sort_mode == 'amp':
            # return  np.argsort(np.absolute(self.amplitudes))[::-1]
            amp = self.amplitudes * np.array([np.linalg.norm(self.modes[:, i], 2) for i in
                                        range(self.modes.shape[1])])
            return  np.argsort(np.absolute(amp ))[::-1]
            ...
            return np.argsort(self._power)[::-1]   
        elif sort_mode == 'energy':
            return np.argsort(self._energy)[::-1]  
        else:
    ...
    def _compute_amplitudes(self, modes, snapshots, eigs, opt):
        ...
        if opt:
            ...
            self.sp_vander = vander
            self.P = P
            self.q = q
            self.sp_s = np.trace(np.dot(np.diag(s).T, np.diag(s)))
            # b optimal
    ...
    def plot_recons_error(self, snaps=None, num=0, idx=None, predict=False, save_fig=False, png_name='test.png'):
        ...
        dmd_tools.plot_recons_error(recons_data, snaps, predict)
        if save_fig:
            fig1 = fig1 = plt.gcf()
            fig1.savefig(png_name)
    ...
    def plot_grid(self, plot_idx, sort_mode='cri16', nx=None, ny=None, cmap=colormap_dmd(), draw_droplet=False, dy_norm=False,  
        ...
        if self.ny is not None:
            ...
            self.ny = ny
        if dy_norm:
            dy = self.dynamics_norm(norm=True)
        else:
            dy = self.dynamics
        ...
        cols_tag = ['Real', 'Image', 'Dynamics', 'Magnitude','Phase']
        rows_tag = [f"mode {plot_idx[i]}" for i in range(n_modes)]
        amp_idx = self.item_sort_idx(m=sort_mode)
        plot_idx = amp_idx[plot_idx]
        fig_w = (n_item-1)*4+3
        print('test')
        fig, axs = plt.subplots(n_modes, n_item, figsize=(fig_w, n_modes*1), gridspec_kw={'width_ratios': [4, 4, 3, 4, 4]},
        ...
        for i, p in enumerate(plot_idx):
            ...
            value_tmp = np.reshape(value_amp_tmp, (ny, nx,))
            if draw_droplet:
                x_droplet, y_droplet = dmd_tools.ball_2d(nx/2, ny/2, )
            ...
            im0.append(axs[i, 0].pcolormesh(v_0 , norm=p_norm_0, cmap=cmap))
            ...
            cbar = fig.colorbar(im0[i], ax=axs[i, 0], shrink=1, pad=0)
            
            # p_norm_1=None
            im1 = axs[i, 1].pcolormesh(v_tmp_imag, norm=p_norm_1, cmap=cmap)
            axs[i, 1].set_yticklabels([])
            axs[i, 1].set_xticklabels([])
            cbar = fig.colorbar(im1, ax=axs[i, 1], shrink=1,  pad=0)
            ...
            axs[i, 2].plot(tt, dy[p].real, c='C0', label='Re')
            axs[i, 2].plot(tt, dy[p].imag, c='C3', label='Imag')
            if dy_norm:
                dy_min, dy_max = (-1, 1)
            else:
            ...
            axs[i, 2].tick_params(direction='in')

            im3 = axs[i, 3].pcolormesh(abs(value_tmp), cmap=cmap)
            axs[i, 3].set_yticklabels([])
            axs[i, 3].set_xticklabels([])
            cbar = fig.colorbar(im3, ax=axs[i, 3], shrink=1,  pad=0)
            phase_angle_tmp = np.reshape(phase_angle[:, p], (ny, nx))
            
            im4 = axs[i, 4].pcolormesh(phase_angle_tmp, cmap='plasma')
            axs[i, 4].set_yticklabels([])
            axs[i, 4].set_xticklabels([])
            cbar = fig.colorbar(im4, ax=axs[i, 4], shrink=1,  pad=0)
        ...
        plt.subplots_adjust(top = 0.98, bottom = 0, right = 1, left = 0.02, 
    ...
    def plot_freq(self, n_sig=None, sort_mode='cri16', figsize=(8, 4), title='', no_neg=True, 
                  save_fig=False, png_name='test.png'):
        # max_amp = am
        ...
        # else:
        #     y = self.amplitudes/y_max
        if sort_mode == 'amp':
            y_label = 'Normalized amplitude'
        if sort_mode == 'cri16':
            y_label = 'Normalized amplitude(sort by criterion16)'
        if sort_mode == 'eigmag':
            y_label = 'Normalized amplitude(sort by eig magnitude)'
        else:
            y_label = f'Normalized amplitude(sort by {sort_mode})'
        ...
        if no_neg:
            ...
            markerline, stemlines, _  = ax.stem(self.frequency, y, use_line_collection=True)
        line = ax.get_lines()
    ...
    def plot_growthrate(self, n_sig=None, sort_mode='amp', figsize=(8, 4), title='',
                        save_fig=False, png_name='test.png'):
    ...
    def plot_save_original(self, save_path_dir, tp2=None, prefix=None, save_img=False, time_step=None, 
                            cm=colormap_dmd(), N_minmax=None,
                            sym_norm=False, cal_minmax=True, vmin=None, vmax=None, save_num=None, Nc=1, 
                            title=None, norm=False, nproc=1, MP=False):
        print("saving original data...")
        ## save recons png
        data = np.reshape(self._snapshots, (self.nx, self.ny, -1)).T
        # data = self._snapshots.reshape
        if cal_minmax:
            value, _vmin, _vmax = comm_tools.save_png_assemble([data], minmax=True)
            if vmin is None:
                vmin = _vmin
            if vmax is None:
                vmax = _vmax
        else:
            value = comm_tools.save_png_assemble([data], minmax=False)
    ...
    # def plot_save_partialrecons(self, save_path_dir, tp2=None, prefix=None, save_img=False, time_step=None, cm=colormap_dmd(), N_minmax=None,
    #                             sym_norm=False, cal_minmax=True, vmin=None, vmax=None, sort_mode=None, save_lead_mode_num=None, Nc=1, 
    #                             title=None, norm=False, nproc=1, MP=False):
    #     print("saving partial recons data...")
    #     ## save recons png
    #     sort_idx = self.item_sort_idx(m=sort_mode)
        
    #     for num_mode in range(save_lead_mode_num):
    #         idx = sort_idx[:num_mode]
    #         print("saved recons with mode idx: {idx}")
    #         if cal_minmax:
    #             value, _vmin, _vmax = comm_tools.save_png_assemble([self.partial_reconstructed_data(idx=idx)], minmax=True)
    #             if vmin is None:
    #                 vmin = _vmin
    #             if vmax is None:
    #                 vmax = _vmax
    #         else:
    #             value = comm_tools.save_png_assemble([self.partial_reconstructed_data(idx=idx)], minmax=False)
    ...
    #         if time_step is None:
    #             time_step = dmd_tools.time_step_str(self.dmd_timesteps) 
    #         savepath = os.path.join(save_path_dir, f'recons_leading_{num_mode}')
    #         comm_tools.make_dir(savepath)
    #         # param_tuple = zip(value, png_save_path, time_step)
    #         # dmd_tools.save_png_plt_par_std(nproc=6, Nc=1, vmin=vmin, vmax=vmax, save_img=True, tp2=tp2, title='', 
    #         #                             param_tuple=param_tuple, cm='Spectral')
    #         dmd_tools.plot_save_png(value, save_path=savepath, time_step=time_step, sym_norm=sym_norm,
    #                                 tp2=tp2, Nc=Nc, vmin=vmin, vmax=vmax, save_img=save_img, title=title, cm=cm,
    #                                 MP=MP, nproc=nproc)
        
    def plot_save_partialrecons(self, save_path_dir, prefix=None, save_img=False, time_step=None, sort_mode=None, cal_minmax=False,
                                sym_norm=False, vmin=None, vmax=None, tp2_suffix='', 
                                save_num=None, recons_mode_idx=None,
                                Nc=1, title=None,
                                norm=False, cm=colormap_dmd(), nproc=1, MP=False):

        print('saving recons by separating modes')
        mode_idx = recons_mode_idx
        
        # print(f'save No.{i} mode with index {k} separately')
        if cal_minmax:
            data, _vmin, _vmax = comm_tools.save_png_assemble([self.partial_reconstructed_data(idx=mode_idx)], minmax=True, 
            ...
            data = comm_tools.save_png_assemble([self.partial_reconstructed_data(idx=mode_idx)], minmax=False, 
                                                            clean=True, clean_threshold=0.1)
        ...
        if not norm:
            ...
            vmax = None
        if prefix is None:
            png_save_path = [os.path.join(save_path_dir, f"mode_sep_recons{len(mode_idx):0{digs}}", f"recons_lead_mode{j:0{digs_t}}")
                            for j in range(t)]
        else:
            png_save_path = [os.path.join(save_path_dir, f"mode_{len(mode_idx):0{digs}}", f"{prefix[j]}")for j in range(t)]
        comm_tools.make_dir(os.path.dirname(png_save_path[0]))
        
        # tp2 = [f'mode {len(mode_idx):0{digs}} ({tp2_suffix})'] * t
        tp2 = [f'combination of {len(mode_idx):0{digs}} modes'] * t
        # print(len(data))
        ...
        dmd_tools.plot_save_png(data=data, save_path=png_save_path, time_step=time_step, sym_norm=sym_norm,
                                tp2=tp2, Nc=Nc, vmin=vmin, vmax=vmax, save_img=save_img, title=title, cm=cm,
                                MP=MP, nproc=nproc)
        # png_save_path_lst.append([f+'.png' for f in png_save_path])
    ...
    def plot_save_mode_sep(self, save_path_dir, prefix=None, save_img=False, time_step=None, sort_mode=None,
        ...
        for i, k in idx_SepMode:
            ...
            data, vmin, vmax = comm_tools.save_png_assemble([self.partial_reconstructed_data(k)], minmax=True, 
                                                            clean=True, clean_threshold=0.1)
            # data = comm_tools.save_png_assemble([self.partial_reconstructed_data(k)], minmax=False, 
            #                                                 clean=True, clean_threshold=0.1)
            # print(data[-1])
            t =  len(data)
            digs_t = comm_tools.count_digits(t)
            ...
            dmd_tools.plot_save_png(data=data, save_path=png_save_path, time_step=time_step, sym_norm=sym_norm,
                                    ...
                                    MP=MP, nproc=nproc)
            png_save_path_lst.append([f+'.png' for f in png_save_path])
        return png_save_path_lst
    
    def plot_dynamics(self, Nr, Nc, sort_mode=None, plot_mode='grid', save_img=False, norm=False, 
                      save_fig=False, png_name='test.png'):
        '''
        ...
        if plot_mode == 'grid':
            ...
            tt = self.dmd_timesteps
            if norm:
                dynamics_real = self.dynamics_norm(norm=True)[idx[:Nr*Nc], :].real
                dynamics_imag = self.dynamics_norm(norm=True)[idx[:Nr*Nc], :].imag
            else:
            ...
            fig, axs = plt.subplots(nrows=Nr, ncols=Nc, figsize=(Nc*3.5, Nr*2),
                                    # subplot_kw={'xticks': [], 'yticks': []},
                                    # sharex='col',
                                    # sharey='row',
            ...
            for i in range(min(self.dynamics.shape[0], Nc*Nr)):
                ...
                # print('test')
                axs[i].plot(tt, dynamics_real[i], color='C0', label='Re',)
                axs[i].plot(tt, dynamics_imag[i], color='C3', label='Imag')
                axs[i].set_title(f"mode {i}", fontsize=18)
            ...
            # plt.tight_layout()
            plt.subplots_adjust(top = 0.98, bottom = 0, right = 1, left = 0.01, 
                                hspace = 0.3, wspace = 0)
            plt.margins(0,0)
            if save_fig:
                fig1 = plt.gcf()
    ...
    def mode_dynamics_video(self, savepath, n_plot=2, sort_mode=None, save_img=False, nproc=1, cmap=colormap_dmd()):
        ...
        if nproc > 1:
            ...
            MP=True
            par = partial(cmap, dmd_tools.mode_dynamics_video, self.nx, self.ny, save_img, self.modes, self.dynamics, MP)
            pool = Pool(nproc)
    ...
    def compare_mode_cri(self, N=10, save_fig=False, png_name='test.png', save_err=False, err_name='test.npz', plot_fig=True):
        ...
        power_idx  = self.item_sort_idx(m='power')[:N]

        sort_method = ['cri16', 'amp', 'power']
        
        x = [i for i in range(self._snapshots.shape[-1])]
        
        
        for idx in [cri16_idx, amp_idx, power_idx]:
        ...
        if save_err:
            np.savez(f'compare_err_{N}.npz', L1_err=L1_err, L2_err=L2_err)
        ...
        if plot_fig:
            ...
            axs.set_ylabel(r'$\frac{\left\Vert X-X_{recons}  \right\Vert_p}{\left\Vert X \right \Vert_p}$', 
                            fontsize=20)
            axs.set_xlabel('Time step', fontsize=15)
            handles, labels = axs.get_legend_handles_labels()
            handles = np.reshape(handles, (-1, 2))
            labels = np.reshape(labels, (-1, 2))
            # plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
            plt.legend(loc='upper right')
            plt.title(f'Reconstruction error (N={N})')
            if save_fig:
                fig1 = plt.gcf()
                fig1.savefig(png_name)
            plt.show()

...
from .spdmd_alt import admm_for_dmd_par
...
def do_dmd(D, t, gamma_vec, nproc=2, rho=1, maxiter=10000, eps_abs=1e-6, eps_rel=1e-4):
    ...
    b_opt = solve(P, q)
    if nproc == 1:
        answer = admm_for_dmd(P, q, s, gamma_vec, rho=rho, maxiter=maxiter, eps_abs=eps_abs, eps_rel=eps_rel)
    else:
        answer = admm_for_dmd_par(P, q, s, gamma_vec, rho=rho, maxiter=maxiter, eps_abs=eps_abs, eps_rel=eps_rel, nproc=nproc)
    return answer, P, q, s, Psi, Phi, D_dmd
...
def admm_for_dmd(P, q, s, gamma_vec, rho=1, maxiter=10000, eps_abs=1e-6, eps_rel=1e-4):
    ...
    for i,gamma in enumerate(gamma_vec):
        ...
        print(f"idx {i} with {step} iterations")
